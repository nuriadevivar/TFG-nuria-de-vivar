...
# OJO: no usar engagement_total_post como feature
feature_cols = ["marca", "anio", "mes_num", "tipo_post"]

X = df[feature_cols].copy()
y = df[target_col].astype(int)

categorical_features = ["marca", "tipo_post"]
numeric_features = ["anio", "mes_num"]
...
results_df.to_csv(os.path.join(METRICS_DIR, "instagram_models_metrics.csv"), index=False)

with open(os.path.join(METRICS_DIR, "instagram_models_metrics.json"), "w", encoding="utf-8") as f:
...
importance_df.to_csv(os.path.join(REPORTS_DIR, "instagram_rf_feature_importance.csv"), index=False)
...
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, "instagram_rf_feature_importance.png"), dpi=300)
plt.close()

print("\nScript de modelado Instagram completado.")