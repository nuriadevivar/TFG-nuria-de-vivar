...
models = {
    "logistic_regression_main": logistic_model,
    "random_forest_main": rf_model
}
...
results_df.to_csv(os.path.join(METRICS_DIR, "capa3_supervised_metrics_main.csv"), index=False)

with open(os.path.join(METRICS_DIR, "capa3_supervised_metrics_main.json"), "w", encoding="utf-8") as f:
    json.dump(results_df.to_dict(orient="records"), f, ensure_ascii=False, indent=4)

print("\n--- MÉTRICAS TRAIN / TEST MAIN ---")
print(results_df)
...
importance_df.to_csv(os.path.join(REPORTS_DIR, "capa3_rf_feature_importance_main.csv"), index=False)
...
plt.title("Top variables más importantes - Random Forest Capa 3 MAIN")
plt.xlabel("Importancia")
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, "capa3_rf_feature_importance_main.png"), dpi=300)
plt.close()

print("\nScript de clasificación capa 3 MAIN completado.")